...
using Lykke.Job.BitcoinCash.Models;
...
using Lykke.Service.BitcoinCash.API.Services.Binder;
...
namespace Lykke.Job.BitcoinCash
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            try
            {
                ...
                {
                    options.DefaultLykkeConfiguration("v1", "BitcoinCash.Job");
                });
                ...
                {
                    new BitcoinCashJobModule (appSettings.Nested(x => x.BitcoinCashApi), Log),
                    new RepositoryModule(appSettings.Nested(x=>x.BitcoinCashApi), Log),
                    new ServiceModule(appSettings.Nested(x=>x.BitcoinCashApi), Log)
                };
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            try
            {
                ...
                app.UseLykkeMiddleware("BitcoinCash.Job", ex => new ErrorResponse {ErrorMessage = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, IReloadingManager<AppSettings> settings)
        {
            ...
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                    AzureTableStorage<LogEntity>.Create(dbLogConnectionStringManager, "BitcoinCashJobLog", consoleLogger),
                    consoleLogger);
